...
echo.
echo  --------------------------------------------------------------------
echo   Event Planner ^& Approval System — Phase 5: Search, Audit ^& Optimization
echo  --------------------------------------------------------------------
echo.
...
echo.
echo [7/7] Seeding Phase 5 (Search ^& Audit) sample data...
python seed_phase5.py
if %errorlevel% neq 0 (
    echo  [WARNING] Seeding failed. You may need to create users manually.
) else (
    echo        Phase 5 Seed data (60+ events) injected successfully.
)

:: ─── Step 8: Launch Server ──────────────────────────────────
echo.
echo  --------------------------------------------------------------------
echo   [SUCCESS] Phase 5 Environment Ready!
echo  --------------------------------------------------------------------
echo.
...
echo   - Indian Rupee (₹) Currency Formatting
echo   - Global Search ^& Pagination
echo   - Admin Audit Logging (/admin/audit-logs)
echo.
